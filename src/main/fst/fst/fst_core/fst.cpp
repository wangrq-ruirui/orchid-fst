...
TLOG_SETUP(COMMON_NS,OutputStream);
TLOG_SETUP(COMMON_NS,FileOutputStream);
TLOG_SETUP(COMMON_NS,DataBlock);
TLOG_SETUP(COMMON_NS,MMapDataBlock);
TLOG_SETUP(COMMON_NS,MemoryLock);
...
bool MemoryLock::LockMemory(void *pData, size_t nLength)
{
    int err = mlock(pData, nLength);
    if (err) {
        char buf[64];
        char *err = strerror_r(errno, buf, 64);
        TLOG_LOG(ERROR, "mlock(%p, %zu) failed: errno=%d, message='%s', run 'ulimit -l' to see if mem lock limit is too small",
                     pData, nLength, errno, err);

        struct rlimit rl;
        if (0 == getrlimit(RLIMIT_MEMLOCK, &rl)) {
            TLOG_LOG(INFO, "(RLIMIT_MEMLOCK) rlim_cur: %lu, rlim_max: %lu", rl.rlim_cur, rl.rlim_max);
        }
        else {
            TLOG_LOG(INFO, "getrlimit(RLIMIT_MEMLOCK) failed, errno=%d", errno);
        }
    }
    return err == 0;
}

bool MemoryLock::UnlockMemory(void *pData, size_t nLength) {
    if (pData == NULL) {
        return false;
    }
    int err = munlock(pData, nLength);
    if (err) {
        char buf[64];
        char *err = strerror_r(errno, buf, 64);
        TLOG_LOG(ERROR, "munlock(%p, %zu) failed: errno=%d, message=%s",
                     pData, nLength, errno, err);
    }
    return err == 0;
}

bool MemoryLock::GetLimit(size_t &nSoftLimit, size_t &nHardLimit) {
    struct rlimit rl;
    if (0 == getrlimit(RLIMIT_MEMLOCK, &rl)) {
        nHardLimit = rl.rlim_max;
        nSoftLimit = rl.rlim_cur;
        return true;
    }
    else {
        TLOG_LOG(ERROR, "getrlimit(RLIMIT_MEMLOCK) failed, errno=%d", errno);
        return false;
    }
}

MMapDataBlock::~MMapDataBlock()
{
    if (m_pData != NULL) {
        UnlockMemory();
        Close();
...
bool MMapDataBlock::Sync() {
    return msync(m_pData, m_nLength, MS_SYNC) == 0;
}

bool MMapDataBlock::OpenRead(const char *pszFilePath, bool fMapPrivate) {
    return Open(pszFilePath, PROT_READ, fMapPrivate ? MAP_PRIVATE : MAP_SHARED, O_RDONLY);
}

bool MMapDataBlock::OpenReadWrite(const char *pszFilePath) {
    // return Open(pszFilePath, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, O_RDWR);
    return Open(pszFilePath, PROT_READ | PROT_WRITE, MAP_SHARED , O_RDWR);
}

bool MMapDataBlock::Open(const char *pszFilePath, int mmapProt, int mmapFlags, int fileFlags) {
    m_fd = open(pszFilePath, fileFlags);
    if (m_fd < 0) {
        TLOG_LOG(ERROR, "open() failed, file='%s', errno=%d", pszFilePath, errno);
        return false;
    }

    m_nLength = lseek(m_fd, 0, SEEK_END);
    lseek(m_fd, 0, SEEK_SET);

    m_pData = (uint8_t*)mmap(0, m_nLength, mmapProt, mmapFlags, m_fd, 0);
    if (m_pData == MAP_FAILED) {
        TLOG_LOG(ERROR, "mmap() failed, file='%s', length=%zu, errno=%d", pszFilePath, m_nLength, errno);
        return false;
    }
...
bool MMapDataBlock::Close() {
    if (munmap(m_pData, m_nLength) != 0) {
        return false;
    }
...
bool MMapDataBlock::LockMemory()
{
    return MemoryLock::LockMemory(m_pData, m_nLength);
}

bool MMapDataBlock::UnlockMemory()
{
    return MemoryLock::UnlockMemory(m_pData, m_nLength);
}
...
bool FileOutputStream::Open(const std::string &strFileName)
{
    fd = open(strFileName.c_str(), O_RDWR|O_CREAT, 00644);
    if (fd < 0)
        return false;
    return true;
}
...
bool FileOutputStream::Write(const uint8_t *pData, size_t nSize)
{
    ...
    while(dumpSize > 0)
    {
        realSize = write(fd, pTmpBuf, dumpSize);
        if(realSize < 0)
            return false;
        dumpSize -= realSize;
        pTmpBuf += realSize;
...
void FstWriteNode::Dump(OutputStream* outputStream, bool hasOutput) {
    /**
